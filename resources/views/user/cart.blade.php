@section('title', 'Donair Spice | Cart Page')
...
function Delete() {
    var id = 1;
    $.post('{{ route('cart.delete') }}', {'id': id,'_token': $('input[name=_token]').val()}, function (data) {
        $("#cart_re").html(data);
        $("#cart_num").load(location.href + " #cart_num");
        $(".nesto12").load(location.href + " .nesto12");
    });
}

function Qty(rowId) {
    var qty = $("#qty_id").val();
    var qty_1 = $("#qty_value").val();
    ...
    $.post('{{ route('cart.qty') }}', {'rowId': rowId,'qty': qty,'_token': $('input[name=_token]').val()}, function (data) {
        $("#cart_re").html(data);
        $("#cart_num").load(location.href + " #cart_num");
        $(".nesto12").load(location.href + " .nesto12");
         if($(window).width()<456){
...
function Coupon() {
	$("#coupon_m").show();
    setTimeout(function(){ $("#coupon_m").hide(); }, 3000);
}
